...
var opt = require('node-getopt').create([
  ...
  [
    'r',
    'repl=ARG',
    'open the repl as "sugar", "slisp", or "score"'
  ],
  [
    'e',
    'exec',
    'compile and then execute source files'
  ],
  [
    'w',
    'watch',
  ...
  [
    't',
    'to=ARG',
    'compile to "tokens", "core" or "json" (compiles to javascript by default)'
  ],
  [
    'b',
    'browser-bundle',
...
(function(___monad) {
  ...
  return mBind((((opt.argv.length === 0) && ((Object.keys(opt.options).length === 0) || opt.options['repl'])) ?
    (function() {
      ...
      setTimeout(function() {
        return ((input.bytesRead === 0) ?
          (function() {
            ...
            return repl.runrepl(opt.options['repl']);
          })() : undefined);
      }, 20);
      return null;
    ...
    return mBind(((true === opt.options['version']) ?
      ...
      ((true === opt.options['browser-bundle']) ?
        (function() {
          ...
          var bundlemin = require.resolve("sugarlisp/lib/sugarlisp-browser-bundle-min.js.gz");
          fs.createReadStream(bundlemin).pipe(fs.createWriteStream("sugarlisp-browser-bundle-min.js.gz"));
          return null;
        })() :
        ((true === opt.options['exec']) ?
          (function() {
            var infile = (opt.argv[0] ?
              (((opt.argv[0].indexOf('.score') === -1) && (opt.argv[0].indexOf('.slisp') === -1) && (opt.argv[0].indexOf('.sugar') === -1) && (opt.argv[0].indexOf('.js') === -1)) ?
            ...
            (function() {
              ...
              watch.watchTree(cwd, {
                filter: function(f, stat) {
                  return (stat.isDirectory() || (f.indexOf('.score') !== -1) || (f.indexOf('.slisp') !== -1) || (f.indexOf('.sugar') !== -1) || (f.indexOf('.slml') !== -1) || (f.indexOf('.slss') !== -1) || (f.indexOf('.slon') !== -1) || (f.indexOf('.ssml') !== -1) || (f.indexOf('.sson') !== -1) || (f.indexOf('.ssss') !== -1));
                },
                ignoreDotFiles: true,
        ...
        return mBind((function() {
          ...
          ((!outfile) ?
            (function() {
              return (function() {
                ...
                return outfile = (fnameonly + (((ext === ".score") || (ext === ".slisp") || (ext == ".sugar")) ?
                  ".js" :
                  (((ext === ".slml") || (ext === ".ssml")) ?
                    ".html" :
                    (((ext === ".slss") || (ext === ".ssss")) ?
          ...
          return (function() {
            return ____mResult((function() {
              console.log('SugarLisp', ['v', packageJson.version, ':'].join(''), 'compiling', infile, 'to', outfile);
